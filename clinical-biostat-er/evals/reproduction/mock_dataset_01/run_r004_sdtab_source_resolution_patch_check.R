...
source(file.path(bundle_root, "skills", "er-statistical-modeling", "scripts",
                 "er_statistical_modeling_helpers.R"))

expected_sdtab <- normalizePath(file.path(study_root, "Models",
                                          "sdtab1062.txt"),
                                mustWork = TRUE)
resolved_sdtab <- core5_resolve_pointer_file(file.path(study_root, "Models",
                                                       "sdtab1062"))
resolved_sdtab <- if (is.na(resolved_sdtab)) NA_character_ else
  normalizePath(resolved_sdtab, mustWork = TRUE)
...
generated <- utils::read.csv(generated_path, stringsAsFactors = FALSE,
                             check.names = FALSE)
joined <- merge(reference, generated,
                by = c("Endpoint", "Stratification", "Dose"),
                suffixes = c("_reference", "_generated"),
                all = TRUE, sort = FALSE)
if (nrow(joined) != 6) {
...
metric_row <- function(metric, value, expected, tolerance = 0) {
  status <- if (is.character(value) || is.character(expected)) {
    if (identical(as.character(value), as.character(expected))) "pass" else "fail"
  } else if (abs(as.numeric(value) - as.numeric(expected)) <= tolerance) {
    "pass"
  } else {
    "fail"
  }
...
summary <- do.call(rbind, list(
  metric_row("runtime_resolved_sdtab_path", resolved_sdtab, expected_sdtab),
...
assessment <- data.frame(
  question = c(
    "r004_sdtab_source_resolution_status",
  ...
  answer = c(
    ...
    "rerun Case38 checker and then continue remaining reference result parity work"
  ),
  evidence = c(
    paste0("resolved_sdtab=", resolved_sdtab,
           "; expected_sdtab=", expected_sdtab),
    ...
    "This check only closes the R004 sdtab source-resolution issue, not full semantic parity."
  ),
  stringsAsFactors = FALSE
)
...
row_path <- file.path(out_dir,
                      "r004_sdtab_source_resolution_row_checks.csv")
assessment_path <- file.path(out_dir,
...
cat("R004 sdtab source-resolution patch check written\n")
